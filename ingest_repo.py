...
def get_or_create_pinecone_index(pc: Pinecone):
    ...
    if PINECONE_INDEX_NAME not in pc.list_indexes().names():
        print(f"Pinecone index '{PINECONE_INDEX_NAME}' not found. Creating a new one...")
        pc.create_index(
            name=PINECONE_INDEX_NAME,
            dimension=VECTOR_DIMENSION,
            metric="cosine", # Cosine is great for text similarity
            spec=ServerlessSpec(cloud="aws", region="us-east-1") # Use a serverless spec for free tier
        )
        print("Index created successfully. It might take a moment to be ready.")
        time.sleep(60) # Give the index time to initialize
    return pc.Index(PINECONE_INDEX_NAME)

# --- DATA FETCHING ---
def fetch_repo_docs(repo):
    """Fetches all Markdown and text files from the repository."""
